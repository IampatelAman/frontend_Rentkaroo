...
const ListingDetails = () => {
  ...
  const getListingDetails = async () => {
    try {
      const response = await fetch(
        `https://rentkaroo-backend.onrender.com/api/listing/${listingId}`,
        {
          method: "GET",
        }
      )
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch("https://rentkaroo-backend.onrender.com/api/booking/create", {
        method: "POST",
        headers: {
      ...
      <div className="px-5 py-10 lg:px-12">
        ...
        <div className="flex flex-wrap gap-2.5 my-5">
          {listing?.listingPhotoPaths?.map((item, index) => (
            <img
              src={`https://rentkaroo-backend.onrender.com/${item.replace("public", "")}`}
              alt="listing photo"
        ...
        <div className="flex gap-5 items-center">
          <img
            src={`https://rentkaroo-backend.onrender.com/${listing?.creator?.profileImagePath.replace(
              "public",
              ""
            )}`}
            alt=""
